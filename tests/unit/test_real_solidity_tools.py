...
def test_run_slither_nonzero_exit_with_findings_is_ok(monkeypatch, tmp_path):
    # Slither exits non-zero when it finds issues; that is success, not failure.
    repo = tmp_path / "repo"
    repo.mkdir()
    run_dir = tmp_path / "runs" / "run-1"
    ...
    def fake_run(command, cwd, timeout=60, env=None):
        json_path = command[-1]
        with open(json_path, "w", encoding="utf-8") as handle:
            json.dump({"success": True, "results": {"detectors": [{"check": "reentrancy-eth"}]}}, handle)
        return CommandResult(command=command, cwd=str(cwd), return_code=255, stdout="", stderr="")

    monkeypatch.setattr("sentinel.tools.static.shutil.which", lambda name: "/fake/slither")
    monkeypatch.setattr("sentinel.tools.static.run_command", fake_run)
...
def test_run_slither_real_failure_surfaces_stderr(monkeypatch, tmp_path):
    repo = tmp_path / "repo"
    repo.mkdir()
    run_dir = tmp_path / "runs" / "run-1"

    def fake_run(command, cwd, timeout=60, env=None):
        # No JSON written and non-zero exit -> a genuine failure.
        return CommandResult(command=command, cwd=str(cwd), return_code=1, stdout="", stderr="solc not found in PATH")
    ...
    assert "solc not found" in (output.message or "")
